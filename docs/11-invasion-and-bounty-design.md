...
**Placement decision (2026-09-08):** sky bases, water bases and terrain-isolated bases remain eligible even when ordinary raids cannot reach them. Prefer a validated approach, but allow a bounded search for a verified safe surface inside that same base when the approach is unavailable. Report the in-base fallback explicitly. Base elevation is not terrain height: do not lower the base to the ground, widen a failed trace, spawn ground NPCs into unsupported air/water or teleport an existing attacker. If neither approach nor safe in-base placement can be established, report that base unavailable without blocking the other bases or calling it a native engine fault.
