...
The new explicit `test-native blueprint` experiment supplies a real `PalIncidentDynamicParameterInvader` with the exact target GUID to the existing manager Blueprint event. It uses the pinned [`StaticConstructObject` binding](https://github.com/UE4SS-RE/RE-UE4SS/blob/2281fa311e417b1dfddedbcd49972d764fddb244/UE4SS/src/Mod/LuaMod.cpp#L1906-L2014), checks the current small pointer-only wrapper layout, and passes only the two actual input arguments. The cooked function also has a local result pointer; the pinned [call marshaler skips non-parameter properties](https://github.com/UE4SS-RE/RE-UE4SS/blob/2281fa311e417b1dfddedbcd49972d764fddb244/UE4SS/src/LuaType/LuaUObject.cpp#L157-L174), so that local must not be supplied as an argument.

The existing private live signature records independently confirm all four pointer offsets: 0, 8, 16 and 24 bytes.

This is a deliberate single-base experiment, not a proven replacement for public march: the skipped native caller's additional bookkeeping/preparation effects remain unknown. PED does not manufacture manager-map entries or set native incident flags. A returned object is retained privately for world/base/new-enemy correlation even when it is absent from the manager's map; a null return is reported as a no-incident outcome, not an invalid target or a successful raid. Before dispatch, a bounded scan of the incident system's waiting/executing/resident collections records old identities, including incidents absent from the manager map. Incomplete identity coverage prevents attribution, not the native request. Blueprint confirmation requires live enemy evidence from the actual returned instance, not an unrelated start callback. Neither outcome automatically launches another route.
