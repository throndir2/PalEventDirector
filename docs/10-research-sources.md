...
### Current-build executable Blueprint inspection

The 2026-09-06 follow-up extracted six specific manager/incident/table assets from the installed build `24575149` pak, SHA-256 `bffab47cbd3b3c6d14d616376d4e0b060b2429a5eb4c2022820d4f38d36a0770`. Verified repak 0.2.3 performed read-only extraction, and a private headless UAssetAPI 1.1.0 decoder read UE5.1 assets using [UsefulFiles mappings at `0e4ae19a05ba0d9fb95d859c09b28f168cb3624f`](https://github.com/PalworldModding/UsefulFiles/tree/0e4ae19a05ba0d9fb95d859c09b28f168cb3624f). Extracted assets and executable-bytecode JSON remain private and are not shipped with PED. This is evidence from the installed game, unlike PMK's generated C++ bodies or dummy editor Blueprints.

| Current Blueprint boundary | Observed behavior and limitation |
|---|---|
| `BP_PalInvaderManager.RequestIncidentInvaderEnemy_BP` | Passes its configured enemy incident ID, the occurred base and the supplied dynamic parameter to its incident helper. The current default ID is `Invader_Common`; PED leaves selection to that property rather than hard-coding the name. |
| Manager incident helper | Resolves the incident system, registers its owner-filtered listener and calls native `PalIncidentSystem.RequestIncident` with owner **the manager itself**. Returns no incident if the system/result is invalid. A wrong-type result is finished by the game helper; a valid invader-incident result is returned. |
| `BP_PalIncidentInvaderEnemy.GetInvaderStartPoint` | Obtains candidates from native `GetCandidateStartLocations`, chooses one if the array is nonempty, and returns false for an empty array. The candidate-generation implementation remains compiled C++. |
| Base incident target position | Uses a rotated local offset of -100 cm in X, then a vertical trace from +500 to -500 cm. It caches the hit location, or returns zero when the base/trace is invalid. This differs from the plain base-transform origin used by the earlier generic navigation experiment. The numeric trace channel is known, but its semantic channel name has not been established. |
| Incident-system admission | `RequestIncident`, `GenerateIncident` and `IsIncidentBeginAllowed` have no executable Blueprint implementation in the inspected asset. The upstream manager path search and native admission remain unresolved. |
| Initialized enemy creation | The game initializes full character-save parameters and uses `PalCharacterManager.SpawnNewCharacter` with a callback and stock invader controllers. Post-spawn logic establishes controller state, movement/arrival handling and anti-escape behavior. Calling a generic Actor spawn is not equivalent. |

Request 13's eight-minute public-march attempt never exposed an enemy incident. Therefore the downstream empty-candidate rejection is a **possible boundary, not the established cause** of that failure; the enemy Blueprint may not have been reached.

The original explicit `test-native blueprint` experiment supplied a real `PalIncidentDynamicParameterInvader` with the exact target GUID to the existing manager Blueprint event. It uses the pinned [`StaticConstructObject` binding](https://github.com/UE4SS-RE/RE-UE4SS/blob/2281fa311e417b1dfddedbcd49972d764fddb244/UE4SS/src/Mod/LuaMod.cpp#L1906-L2014), checks the current small pointer-only wrapper layout, and passes only the two actual input arguments. The cooked function also has a local result pointer; the pinned [call marshaler skips non-parameter properties](https://github.com/UE4SS-RE/RE-UE4SS/blob/2281fa311e417b1dfddedbcd49972d764fddb244/UE4SS/src/LuaType/LuaUObject.cpp#L157-L174), so that local must not be supplied as an argument.

The existing private live signature records independently confirm all four pointer offsets: 0, 8, 16 and 24 bytes.

That first experiment was deliberately limited to one base. Request 14 proved enemy creation and arrival, but later crashed in native teardown because the manager's `InvaderInfo` actor had never been initialized. The unsafe direct call is now retired. Its private incident-system baselines and bounded observational evidence remain useful, but a returned incident alone is not a complete native raid lifecycle.

If this Blueprint route does not establish a usable native lifecycle, the game-managed `SpawnNPCForServer` reference remains the direct-spawn fallback. Its nil delegate is explicitly supported by the [pinned delegate setter](https://github.com/UE4SS-RE/RE-UE4SS/blob/2281fa311e417b1dfddedbcd49972d764fddb244/UE4SS/src/LuaType/LuaUObject.cpp#L1949-L1956), but Lua call forwarding must retain the trailing nil argument. The current incident-system Blueprint also calls `BP_NPCAIController.ForceBattleStartToTarget` for an ordinary NPC attack, supplying a concrete hostility-control lead. Do not call the potentially oversized character-save-parameter initialization getter directly through the 512-byte Lua wrapper merely because the engine's own Blueprint safely calls it.

### Native raid-state initialization repair

Steam updated IMOUTO to dedicated build `25080279` on 2026-09-07, and the user explicitly approved targeting it. The new Shipping executable SHA-256 is `61c7d285a7a5072486ae099ae7c7c9be5ef0c34d843e06e05517e1f0bd157c02`; the new server pak SHA-256 is `2e6a964a1fe2e8bd7d754648d35240e2c1567e780455aedd22f27dbc9dcedabe`. The old build's native addresses are not reused. Fresh extraction/decoding found unchanged manager, incident-system, base/enemy Blueprint implementations and incident table; only the visitor asset pair differed among the twelve selected files.

Private offline native inspection established the actual initialization path, rather than filling in generated SDK stubs:

| Build-25080279 native boundary | Observed behavior |
|---|---|
| Manager setup at RVA `0x2e5c590` | Deferred-spawns the real native `APalInvaderInfo` class, sets `StartRealTime`, `BaseCampId` and `InvadeGrade`, then finishes actor spawning. Normal preparation/declaration and visitor handling surround this path. |
| Actor `BeginPlay` at `0x2e48b30` | Resolves the world-scoped invasion manager and registers itself through the native routine at `0x2e68d30`. |
| Native registration at `0x2e68d30` | Assigns `manager.InvaderInfo`, binds the actor's start/end-wave delegates to the manager, and emits the created-info notification. |
| Manager update beginning at `0x2e71170` | Waits for the initialized state actor's start condition, obtains the exact base/observer, constructs the typed incident parameter, invokes the game's Blueprint handoff, marks the native observer and adds the returned incident to the manager's own map. |
| Native first-wave transition at `0x2e5f650` | Updates the real state actor's first-wave flag, wave count, maximum waves and broadcast group as the enemy incident starts. PED must not fabricate those values. |

PED now follows that lifecycle through reflected engine APIs: `BeginDeferredActorSpawnFromClass`, the fresh actor's base and control grade, and `FinishSpawningActor`. Its default start condition is confirmed through `GetRemainInvadeStartRealTimeSeconds` before finalization. It does not call a raw RVA, patch the executable, add another loader, call the bare Blueprint handoff, assign `manager.InvaderInfo`, bind replacement wave delegates, or manufacture incident-map entries. The native actor and manager own registration, enemy spawning and teardown.

The explicit one-base `blueprint` control uses grade 1 and an already-due native start condition, not a guessed wall-clock epoch. Startup qualification found that `DateTime` has zero reflected fields in this pin: although the native clock helper is small, its opaque result cannot safely be treated as a Lua `{Ticks=...}` table. The repaired control therefore leaves the newly constructed actor's default timestamp untouched and requires a finite, nonpositive native remaining-time result before finalization. This intentionally starts without the normal declaration wait or a Negotiator request. It is not a replacement for normal all-base/bounty starts.

All new reflected frames are qualified against their exact small layouts, including the double-precision transform, enum collision property and scalar remaining-time return, and remain below the pinned wrapper's 512-byte limit. Metadata qualification is read-only and runs on the game thread at startup. FName identity is compared case-insensitively rather than relying on name-pool display casing. Trailing nil arguments are preserved, so the required null deferred-spawn `Owner` is not accidentally omitted. A pre-existing state actor is not replaced; a failed initialization locks further native work. A new raid is confirmed only from live native enemies correlated with this request's state actor, base and broadcast group.

Request 14 remains interrupted/recovery-required and is never replayed. The earlier private dump and both successful-start/crash evidence captures remain unchanged. A fresh in-world control must establish end-to-end completion on the repaired build before this route is promoted to normal scheduling or bounty dispatch.
