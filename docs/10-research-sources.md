...
PED now follows that lifecycle through reflected engine APIs: `BeginDeferredActorSpawnFromClass`, the three deferred-spawn inputs, and `FinishSpawningActor`. It does not call a raw RVA, patch the executable, add another loader, call the bare Blueprint handoff, assign `manager.InvaderInfo`, bind replacement wave delegates, or manufacture incident-map entries. The native actor and manager own registration, enemy spawning and teardown.

The explicit one-base `blueprint` control uses grade 1 and the authoritative current world time, not a guessed wall-clock epoch. The audited `CalcRealTimeDifferenceToNow` implementation returns original-time minus world-now plus offset; zero inputs therefore provide a safe eight-byte integer-tick clock result. This control intentionally starts without the normal declaration wait or a Negotiator request. It is not a replacement for normal all-base/bounty starts.

All new reflected frames are qualified against their exact small layouts, including the double-precision transform and integer-tick structs, and remain below the pinned wrapper's 512-byte limit. Metadata qualification is read-only and runs on the game thread at startup. Trailing nil arguments are preserved, so the required null deferred-spawn `Owner` is not accidentally omitted. A pre-existing state actor is not replaced; a failed initialization locks further native work. A new raid is confirmed only from live native enemies correlated with this request's state actor, base and broadcast group.
