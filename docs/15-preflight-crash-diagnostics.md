...
Dry-surface and full pawn-response clearance qualification is still incomplete. The candidate therefore returns `dry-clearance-unqualified` even after a walkable support hit and clear ground-channel capsule sweep. A short water-plane miss or a CDO's cached water flag must not be promoted to dry placement. This revision can qualify template geometry through `ClassCatalog`, but cannot yet admit custom NPC spawns through that incomplete surface gate.
