...
The repaired control qualifies the small reflected spawn/finalization/clock layouts on the game thread at startup and again before allocation. It retains the trailing nil `Owner` argument required by deferred spawning. Only the fresh actor's base, start time and control grade are initialized before `FinishSpawningActor`; `BeginPlay` must register that same actor with the same world-scoped manager. Existing raid-state actors are not overwritten. A native fault or failed registration stops further work; no cleanup flags are toggled to hide missing initialization. Request 14 remains interrupted/recovery-required after restart and must not be replayed.
