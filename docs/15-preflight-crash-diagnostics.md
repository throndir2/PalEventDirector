...
Automatic streaming-source ownership is still qualified only in the explicit startup scenarios, not yet coordinated across a normal all-base event. Normal events can therefore report a base physically unavailable; the placement gate is not proof of all-base prewarming, continuous route collision or successful combat.
