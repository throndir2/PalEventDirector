...
The expanded harness separates inspection from mutation. `inspect-native` reads the current base without changing the event or scheduler. The `test-native` route argument selects Boolean admission, ordinary selected-base march, the controller debug RPC, or the current-build Blueprint handoff on the same validated base. The Blueprint comparison creates the typed per-base parameter object, calls the actual Blueprint override, and observes its returned incident without inventing native manager registration. Its eight-minute window and null-result reporting do not imply successful initialization or a raid; see the [current-build findings](10-research-sources.md#current-build-executable-blueprint-inspection). A bounded group-name argument is accepted only by the debug route and goes to native selection without a PED loaded-table veto; names resembling private identifiers are withheld from diagnostic text rather than blocking invocation. Arbitrary methods and wider scopes remain unavailable. No route automatically falls back to another.
