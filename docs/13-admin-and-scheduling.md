...
| `!siege inspect-native` / `!ped inspect-native` | Admin-only inspection of the current base, including occupied slots. Does not request an invasion, reset state, or clear the start guard. Writes detailed evidence and opens a bounded passive observation window. |
...
| `!siege test-native blueprint` / `!ped test-native blueprint` | Repaired native-state control for one base at grade 1. Deferred-spawns the real `APalInvaderInfo`, sets the exact base, and verifies its fresh default start time is already due through the native remaining-time getter before finishing initialization. Palworld owns wave dispatch, Blueprint incident creation, registration and cleanup. No direct Blueprint invocation, opaque timestamp writes, map edits, retry or fallback. |
...
Admin starts carry a persisted `adminOverride` selected by authorization, never from chat text. Their normal selected-base request now uses `StartInvaderMarchForBaseCamp(Guid)`, matching Endless Siege's primary trigger, without its broader fallback calls or startup cleanup. The private Boolean admission helper remains an explicit experimental route. Native acceptance still requires actual correlated enemy-start evidence, not a successful call. Current status prioritizes the latest failed attempt over unrelated historical results.

Chat start requests retain their requester privately for progress: request received, target count, call result, actual native preparation when present, live enemy state and outcome. A zero countdown skips PED's delay, not Palworld's own preparation. Public march may remain `starting` for up to eight minutes by default, while reporting the actual native countdown when available. A successful call is not a guaranteed spawn. Timeout/error details remain bounded and privacy-safe; native faults lock further native calls until investigation and a fresh process.

Per-target replies use stable request-local ordinals, for example `PED #12 target 2/10`, rather than base or player IDs. A native Boolean false is labeled **native rejection with no supplied reason**, not an invalid target. A PED structural failure names the exact missing object or mismatching identity: manager, observer map/entry, observer, base model, model `GetId`, or observer `TargetBaseCampID`. Returned false calls still get a post-call snapshot when no native fault occurred, without retrying the invasion request. State transitions are explicitly observations, not guessed engine causes. Chat shows at most 16 detailed target reports; the private journal retains every result. Unrecognized raw error strings are never echoed into chat.
...
Detailed records remain in `Pal\Saved\PalEventDirector\native-experiments.ndjson`, with native breadcrumbs separate. Passive samples at 1, 5, 15, 30, 60, 120, 300 and 600 seconds can outlive event deadlines but never reopen scoring or replay a request. Active event progress additionally examines at most four requested bases every five seconds, rotating through larger target sets. It reads exact native enemy type, group/base/world identity, execution and live-attacker counts; it does not infer a negotiator from a global count of one body.
