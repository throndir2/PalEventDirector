...
The primary admin command is `!siege start all-bounty [0-60 minutes]`. `all-bounty` now requests a **simultaneous PED-controlled assault**, not Palworld's single-base native raid. At `0`, bounded spawn batches are interleaved across every eligible online-guild base without waiting for another base's fight or acceptance. Default composition is three level-30 stock bounty NPCs per base, rotating the 34-ID catalog. They are directed toward their assigned base and local defenders/buildings, not a distant requesting player. Missing safe placement is reported per base. A returned spawn handle is not confirmation: initialized owned attackers and successful AI setup are required. Authority, positive countdowns, durable intent, resource limits and recovery remain enforced. See [the complete administration reference](docs/13-admin-and-scheduling.md).

Other native-based profiles use the public `StartInvaderMarchForBaseCamp` route used by Endless Siege, with an eight-minute observation window for native declaration/preparation. A start callback or newly correlated executing enemy incident with live attackers confirms the assault; preparation, visitors and old incidents never do. `!siege test-native march` provides the same route at one base without bounty substitution or fanout. PED does not import Endless Siege's all-base/random retries or global startup cancellation.

On qualified dedicated build `25080279`, `!siege test-native blueprint` now initializes the game's real raid-state actor before native incident creation. It is a one-base, native-composition control at grade 1, with an immediate native start time and no bounty substitution. Native `BeginPlay` registers the actor and binds wave handlers; the game creates and registers the enemy incident. PED never calls the unsafe bare Blueprint handoff, manufactures a manager-map entry, or substitutes a dummy cleanup actor. Live enemy evidence is still required; see the [native lifecycle repair](docs/10-research-sources.md#native-raid-state-initialization-repair).

Built-in profiles:

| Profile | Composition |
|---|---|
| `all-bounty` | Default. Simultaneous, base-scoped custom assault using game-initialized stock bounty NPCs; configurable count/level and globally rotating catalog. No native incident or global raid-state actor. |
