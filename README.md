...
The installer configures `PalEventDirectorDeployments\Start-PalEventDirectorImouto.ps1`. Start the IMOUTO dedicated server with that script, not the Steam Play button: it reads the verified build ID from the local Steam dedicated-server manifest and supplies both PED environment variables only to the child server process. Run it with `-ValidateOnly` to verify launch integration without starting Palworld.
